...
Animations use a 4:3 frame in the archive and a 16:10 frame on the article page, with the same thin border as images. Include responsive sizing in the uploaded document. There is no pause control. Animations load when first visible. Scrolling preserves playback; hiding the browser tab or closing the editor unloads the animation. Returning to the tab restarts it. Reduced-motion visitors explicitly choose Play, then the button disappears. Failed loads offer Retry.
...
Click a cover in the feed or article to open the larger cover dialog. Images and HTML animations use the same viewer. Close with Escape, the close button, or the backdrop; the same iframe stays in place when the viewer opens or closes, preserving continuous playback. The existing dialog is promoted to the browser’s top layer without moving or cloning the iframe. Animation viewers have no pause control.
